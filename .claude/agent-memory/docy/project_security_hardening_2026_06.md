...
**Neon dev-branch refresh feature (same branch, 2026-06-20, validate green):** admin tool to reset `dev` Neon branch from `production` at `/admin/db` (Observe/Mirror/Runs tabs). Canonical feature doc = `blueprint/data/neon-branch-refresh.md` (data-plane sibling of `drizzle-workflow.md`). Code: `$lib/server/neon/` (sole `NEON_API_KEY` holder, never returns connection strings — confirmed in module header) + `$lib/server/dbops/` (orchestrator, lazy-advance-on-poll executor, `dbops.run` ledger). Crons `dbops-refresh` (gated by `DBOPS_AUTO_REFRESH_ENABLED`, daily) + `dbops-reaper` (`*/15`, needs Vercel Pro) in `vercel.json`. Key teaching point = two access planes (data plane DSN vs control plane Management API); reset-from-parent has NO SQL equivalent. v1 = Postgres only; polyglot/Neo4j/R2 refresh is design-only — documented as "Future, not in v1".
