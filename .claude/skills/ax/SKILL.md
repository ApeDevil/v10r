...
| Pattern library (README index region + the `docs/pattern-library/` docs section: `*.md` page per pattern, catalog index route) | `mcp/patterns.registry.json` (two-tier: 11 deep cards + ~125 light index rows) | Pages: `scripts/patterns/build-derived.ts` (`patterns:build`; gate = `patterns:check`) → /docs/pattern-library/<id>, RAG, /llms.txt (listed FIRST). Catalog at /docs/pattern-library = live route over `$lib/server/patterns/catalog.ts`, no build step. `pattern-library` is a real 4th DocSection — new enumeration sites must include it. Excerpt allowlist = DEEP tier only (light refs would ~3× the committed snapshot). Chatbot twin: `search_pattern_library` (NOT `search_patterns` — that name is the MCP tool) |
...
- **Negotiation:** clean /docs URL + `Accept: text/markdown` → **303** + `Vary: Accept` + `no-store` (a 308 keyed off a request header without Vary = cache poisoning); `sec-fetch-dest: document` forces HTML — on the `.md` URL too: a browser document navigation (and every SvelteKit data request, which would otherwise feed markdown to the router's JSON.parse) falls through the hook into routing, where `docs/+layout.server.ts` 303s the `.md` suffix away to the rendered page. Agent `.md` hit → 200 `text/markdown` + `s-maxage=3600` + `Vary: Sec-Fetch-Dest` + `Link` canonical/llms-txt. The `docsMarkdown` hook sits at **index 3 of 14** — above every Set-Cookie writer, or shared-cache `s-maxage` dies silently; `handle-chain.gate.test.ts` pins both the 14-name snapshot and the ordering.
