...
- **`docs/pattern-library/` is GENERATED** (from `mcp/patterns.registry.json` by `patterns:build`, step 2 of the refresh chain) and enters the corpus automatically as its own docs section — ~136 short pointer pages with `/docs/pattern-library/<id>` citation paths, hash-skipped like any other doc. If they ever dilute retrieval, the documented rollback is one line: add `docs/pattern-library/` to the ingester's blocked prefixes via `RAG_ONLY_BLOCK`-style exclusion.
