...
Language switching requires full page reload (no runtime switching).
...
<script lang="ts">
  ...
  import { getLocale, availableLanguageTags } from '$lib/paraglide/runtime';
...
<nav aria-label="Language">
  {#each availableLanguageTags as lang}
    <a
      href={lang === 'en' ? $page.url.pathname : `/${lang}${$page.url.pathname}`}
      hreflang={lang}
...
**Important:** `data-sveltekit-reload` forces full page reload for locale switch.
...
<svelte:head>
  ...
  {#each availableLanguageTags as lang}
    <link
      rel="alternate"
      hreflang={lang}
      href="{baseUrl}{lang === 'en' ? '' : `/${lang}`}{$page.url.pathname}"
    />
  {/each}
...
| Language switch | Full page reload (`data-sveltekit-reload`) |
