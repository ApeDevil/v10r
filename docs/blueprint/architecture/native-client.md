...
1. **API-key/bearer guard:** `requireApiKey(request)` in `$lib/server/http/guards.ts` (the reserved seam), plus Better Auth `bearer` plugin enablement with an **exact-match** custom scheme entry in `trustedOrigins` (never a wildcard — trustedOrigins widening is a known account-takeover surface, GHSA-vp58-j275-797x).
