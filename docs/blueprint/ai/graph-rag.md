...
> **Tenancy note (current implementation).** The illustrative `:Entity {name}` model above is **not** how the live RAG graph is keyed. Live `:Chunk` and `:Entity` nodes are per-tenant: `:Chunk` carries `ownerId`, `:Entity` is keyed `{name, ownerId}`, and `scripts/db/setup-neo4j.ts` enforces `entity_name_owner_unique (name, ownerId)` plus `entity_owner` / `chunk_owner` indexes. Every read is scoped `WHERE ownerId IN $ownerIds`. See [layered-rag.md](./layered-rag.md#graph-tenancy-neo4j) for the canonical model.
