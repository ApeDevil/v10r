...
| [layered-rag.md](./layered-rag.md) | **Primary RAG doc.** Two-layer split (llmwiki + retrieval); **one shared `retrieve()` kernel, two profiles/corpora** (chatbot system-docs + deskbot user-files); catalog grounding (`search_catalog` incl. browse/enumerate `query:"*"` mode, `<catalog-map>`, citation chips); docs corpus (`search_project_docs`, system-owned, `db:ingest-docs`) + relevance-gated system-docs prefetch (`shouldGroundFromSystemDocs`); deskbot corpus (`source = 'desk'`, `desk_search_knowledge`/`desk:ask` read-only, `deskbot-retrieval.ts`, `desk-retrieval-sync` freshness); **graph tenancy** (per-tenant `:Chunk`/`:Entity`, owner-scoped reads, `deleteUserGraph` GDPR sweep); tool contracts, read path, citation verification |
