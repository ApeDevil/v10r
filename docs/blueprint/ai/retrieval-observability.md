...
> **Status update (2026-08-13): the rag-chat showcase UI this doc describes was RETIRED** — the retrieval showcase tree (`/showcases/ai/retrieval/*`) and the `rag-demo` surface (`POST /api/ai/showcase/rag`, `dryRun`) were deleted when the AI-surface architecture pages landed. The trace/event **contracts remain live**: the `pipeline:*` metadata frames and `$lib/types/retrieval-trace.ts` (registry, phases, retrievers, `startOffsetMs` semantics) now feed the `/showcases/ai/chatbot` page's recorded/live trace viewer (`$lib/showcases/ai/replay.ts`). Read the design decisions below as the record behind those contracts, not as a live page description.
>
> **Renamed 2026-08-31** by the naming-integrity refactor (see [`naming.md`](../../naming.md)): `pipeline.ts` → `retrieval-trace.ts`, `PipelineStep*` → `RetrievalStep*`, `PIPELINE_REGISTRY` → `RETRIEVAL_STEPS`, `RetrieverLane` → `RetrieverId` (field `lane` → `retriever`), `RetrievalLayer` → `RetrievalCorpus` (in `retrieval-corpora.ts`), and the descriptor's `path` field → `engine`. The prose below uses the new names; the axis *names* in the table are unchanged concepts.

> **Original status: BUILT + validate-green + browser-verified-live (2026-06-27, dev, UNCOMMITTED).** 16-agent task force (ARY · SYS · SVEY · UXY · DATY · APY · RESY · SCOUT, two cross-pollination rounds) → full implementation. P1–P3 done; P4 (true single-run) deferred. Verified: rag-chat type-clean (real `svelte-check`; the project's `check` gate has since been switched to run real `svelte-check` itself), rag-chat 17/17 + server 299/299 tests green, biome clean, zero net new project errors. Browser-proven: waterfall shows the three tiers starting at the same offset (true parallel overlap), wall-clock span total (1981ms, not the summed 2124ms), Step⟷Timeline toggle, Paths/Tokens panes, token Measured-vs-~Estimated honesty, "Run without RAG" counterfactual. Held from the RAG corpus until committed + re-ingested, so the chatbot can't assert it as live. Implementation plan: `~/.claude/plans/retrieval-observability-redesign.md`.

## What this is

The rag-chat showcase teaches how v10r's layered retrieval works **and** doubles as a working chat. Today the trace is fragmented: five mode pills (`Vector` / `Small-to-Big` / `Entity Graph` / `LLM Wiki` / a gamified-unlock `All Three Fused`), a compact "latency bar" rail, and a right-side drawer that most users never open. The bar isn't a real waterfall, the modes fragment one pipeline into five page-states, and the most interesting fact — that the retrieval tiers run **concurrently** — is invisible.

The redesign collapses all of that into **one always-visible "Retrieval Observability" region under the chat** with three coordinated views (Timing · Paths · Tokens), the retrieval modes demoted from page-states to a free post-hoc **focus filter** over a single fused run.

## The core decisions

### 1. One fused run, modes → focus filter

The four retrieval modes were just different `retrievalTiers`/`fusion` flags on **one `retrieve()` call**. Always running fused (`tiers:[1,2,3], fusion:'rrf'`) costs **zero extra generate quota** — tiers run in parallel (`Promise.all`, `retrieval/index.ts:102-146`), embedding is paid once, and there is one generate call per turn either way (only DB/Neo4j load increases). "What did the Entity Graph tier contribute?" becomes a **client-side filter** over the already-captured `tierChunks` — no re-run, no quota spent. The only turn-spending experiment is the counterfactual ("run without RAG").
...
"Layer" was overloaded. There are **three independent axes**, all homed in `src/lib/types/retrieval-trace.ts`:

| Axis | Type | Tags | Members |
|------|------|------|---------|
| **Stage** (temporal) | `RetrievalPhase` | steps | `embed · retrieve · fuse · assemble · generate · verify` |
| **Retriever** (source) | `RetrieverId` | tierChunks keys + retrieve steps | `tier-1 · tier-2 · tier-3 · llmwiki` |
| **Corpus** | `RetrievalCorpus` *(existing)* | chunks | `llmwiki · retrieval · catalog · docs` |

The Timing waterfall lays out by **Stage**; the Paths panel groups/colors by **Retriever**; the admin diagram (`admin/ai/retrieval-pipeline.ts`) uses the **Corpus** axis. `drill` is a step *inside* the `retrieve` phase (its mid-generate timing is expressed by `startOffsetMs`, not its own phase); `wiki`-ness is the `engine` field, not a phase. The corpus axis (`RetrievalCorpus`) moves to a client-safe shared home so both admin and showcase import it (no drift).

## Contract delta (`src/lib/types/retrieval-trace.ts`)
...
- **Closed `phase: RetrievalPhase`** discriminant on every step event + exhaustive `PHASE_OF: Record<PipelineStepId, RetrievalPhase>` (a missing tag is a compile error). The viz switches on `phase`, never string-matches step ids.
- **`RETRIEVAL_STEPS`** (descriptors `{ id, label, phase, engine:'chunks'|'llmwiki'|'both', retriever?, dynamic? }`) replaces `PIPELINE_STEPS` + `LLMWIKI_STEPS`.
- **`instanceKey`** (server-stamped; `= id` except `chunks:drill → drill#${ordinal}`) — stable list keys, avoids the `each_key_duplicate` class of crash.
...
- **`tierChunks`** typed `Partial<Record<RetrieverId, ChunkSummary[]>>` (was `Record<string, …>`); `ChunkSummary.source`/`tier` typed; `retrieverScores` widened to `Partial<Record<RetrieverKind, number>>` (so bm25 stops folding into vector); `survivalReason → dispositionReason`, **always populated** incl. drop reasons (`below_top_k`, `rrf_cutoff`).
...
- **`generate:error`** — the chatbot branch paints it from the streaming helper's `onAttemptFailure` on the FINAL failure (a retried attempt re-opens the bar as `active` instead), with the classified kind as `error` — never the provider's prose; a partial answer keeps the full timing detail with status `error`. A **cancelled** turn (Stop, a closed tab) is not a failure: the terminal closes `done` over what streamed, and the one `[ai:chat:llmwiki] turn cancelled by the client` line before the timing line is its record — the client that cancelled never receives the frame. The desk branch wires the inner `streamText` `onError`; a client `finalizeActive()` backstop flips a lingering `active` → `error` when `chat.status` settles.
...
- **Catalog lane:** a navigation question's pre-generation catalog search (`<catalog-results>`, no embedding) is one dynamic **`catalog`** step in the parallel band, detail `{ hits, surface }`.
- **Per-lane settle times, not barrier times.** The chatbot's retrieval lanes join at one `Promise.allSettled`; each lane's `durationMs` is stamped the moment *that* promise settles (`settle()` in `context-assembly.ts`), so a slow lane never inflates a fast one. The shared query embedding is its own coarse **`embed`** step on grounded turns (appended dynamically; greetings never show it — the query text is not in the detail). The wiki and docs lanes chain on the embed, so their bars include it.
- **The `generate` terminal is emitted once, last** — after the post-text stages, while the message is still open — and carries the turn's timing shape in its detail: `preStreamMs` (orchestrator entry → `start` frame), `steps`, per-step `firstTokenMs`, `tools[{name, ms}]` (from the SDK's tool-call hooks), `finalize{verifyMs, catalogMs, persistMs, budgetMs}` (absent = stage skipped), and `reasoningTokens` (Gemini's `thoughtsTokenCount` — which arm of the thinking A/B the turn ran on). The same numbers go to the server log as one sanitized `[ai:chat:timing]` line per turn. `scripts/perf/vely-turn-tap.js` reads them from the client side.
- **Delivery rule (do not violate):** trace events ship as `message-metadata` frames carrying the **full accumulated array** (REPLACE semantics) — never deltas, never transient `data-*` parts. Because every frame is the whole state, emits are **coalesced**: `flush()` schedules one frame per microtask turn and `flushNow()` drains it before the first model frame and before `finish`, so a burst of settling lanes costs one frame, not one per event. The assistant frame MUST open with an explicit `writer.write({type:'start', messageId})` **before** the first metadata write, and the merged text stream MUST suppress its own start via `toUIMessageStream({ sendStart:false })`. Violating either re-splits the answer into a duplicate empty avatar.
...
- **Generic primitive** `src/lib/components/viz/timeline/Waterfall.svelte` + `viz/timeline/types.ts` — domain-agnostic `WaterfallRow[]` (`{ id, label, startOffsetMs, durationMs, status, color, laneId?, groupId?, depth?, parentId? }`), `orientation` prop, `totalMs` (explicit span). **Must not import `$lib/types/retrieval-trace`** — route adapters map pipeline → row. `cycle/CycleWaterfall.svelte` becomes a thin adapter too.
- **One trace factory** `…/rag-chat/_components/trace/retrieval-trace.svelte.ts` (`createRetrievalTrace` over `RETRIEVAL_STEPS`, `engine`-filtered, `instanceKey`, optional `citations`) — replaces `retrieval/retrieval-trace.svelte.ts` + `llmwiki/llmwiki-trace.svelte.ts`. **Views stay split** (`trace/retrieval/` chunk cards, `trace/llmwiki/` page+citation+graph-path cards — llmwiki provenance has no similarity score, so it needs a distinct card shape).
- **Shell** `…/_components/RetrievalObservability.svelte` (replaces `TraceRail` + `TraceDrawer`): owns the engine toggle, tier focus filter, Step/Timing + layout prefs (localStorage), and the Timing/Paths/Tokens panes. Selection state reuses the factory's existing `selectedStepId`/`selectStep`.
...
- **P2 — Components.** Generic `Waterfall` primitive; `createRetrievalTrace`; `RetrievalObservability` shell + Timing/Paths/Tokens panes + cross-panel selection; delete drawer/rail/ModeSelector.
