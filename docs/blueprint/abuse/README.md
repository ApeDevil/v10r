...
| **[ai-budget.md](./ai-budget.md)** | • Daily token cap (`DAILY_TOKEN_CAP = 100_000`)<br>• Redis key shape: `ai:budget:{userId}:{YYYY-MM-DD}`, 25h TTL<br>• `checkUserBudget` + `chargeTokens` pattern, where enforced<br>• Check-then-charge caveat (v1 known overshoot) |
