...
|--------|---------|---------|
| `db:setup` | composite | Full fresh-DB bootstrap (extensions → migrate → RAG → Neo4j) |
...
podman exec -it v10r bun run db:setup
```

`db:setup` chains: `db:rag-pre` (pgvector extension) → `db:migrate` (schemas, enums, tables) → `db:rag-post` (tsvector, indexes, seed) → `db:neo4j-setup`.

**Schema changes (ongoing development):**
...
**Template users:** Clone the repo, set `DATABASE_URL`, run `db:setup`. The committed migration in `drizzle/0000_init.sql` contains all `CREATE SCHEMA`, `CREATE TYPE`, and `CREATE TABLE` statements in the correct dependency order.

**Note:** The `0000_init.sql` was generated by `drizzle-kit generate` with one manual fix: the `date_format` enum reference on `app.user_preferences` needed its schema qualifier added (`"app"."date_format"`) due to a Drizzle bug where column-name/enum-name collision drops the schema prefix.
