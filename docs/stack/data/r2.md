...
**Client setup:** `$lib/server/store/index.ts` creates an `S3Client` with `region: 'auto'` and endpoint derived from the account ID. Returns `null` when credentials are missing; callers treat that as "storage unavailable".
