...
import {
	...
	moveAssetFolder,
	renameAssetFolder,
} from '$lib/server/blog/asset-folders';
import {
	FolderCycleError,
...
const limiter = createLimiter('rl:blog:asset-folders:mutate', 60, '1 m');

const UpdateAssetFolderSchema = v.object({
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
	...
	const parsed = safeParse(UpdateAssetFolderSchema, body);
	...
	try {
		if (parentId !== undefined) {
			await moveAssetFolder(params.id, user.id, parentId);
		}
		if (name !== undefined) {
			const row = await renameAssetFolder(params.id, user.id, name);
			return apiOk({ folder: row });
		}
		const row = await getAssetFolder(params.id, user.id);
		return apiOk({ folder: row });
...
export const DELETE: RequestHandler = async ({ params, url, locals }) => {
	...
	try {
		const result = await deleteAssetFolder(params.id, user.id, { recursive });
