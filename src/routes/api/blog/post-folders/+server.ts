...
const limiter = createLimiter('rl:blog:post-folders', 10, '1 m');

const CreatePostFolderSchema = v.object({
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const parsed = safeParse(CreatePostFolderSchema, body);
	if (!parsed.success) return apiValidationError(parsed.issues);

	const { name, parentId } = parsed.output;
	try {
		const row = await createPostFolder(user.id, name, parentId ?? null);
