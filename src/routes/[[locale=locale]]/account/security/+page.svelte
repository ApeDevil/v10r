...
function passkeyLabel(p: (typeof data.passkeys)[number]): string {
	return p.name || p.authenticatorLabel || m.app_account_security_passkey_unnamed();
}
	...
	try {
		...
		if (result?.error) {
			if (!isCeremonyCancel(result.error.message)) {
				toast.error(result.error.message ?? m.app_account_security_passkey_add_failed());
			}
		} else {
			toast.success(m.app_account_security_passkey_added());
			newPasskeyName = '';
			await invalidateAll();
		}
	} catch (err) {
		if (!isCeremonyCancel(err instanceof Error ? err.message : undefined)) {
			toast.error(m.app_account_security_passkey_add_failed());
		}
	} finally {
		addingPasskey = false;
	}
...
async function saveRename() {
	...
	if (result.error) {
		toast.error(result.error.message ?? m.app_account_security_passkey_rename_failed());
	} else {
		renamingId = null;
		await invalidateAll();
	}
...
async function deletePasskey() {
	...
	if (result.error) {
		toast.error(result.error.message ?? m.app_account_security_passkey_delete_failed());
	} else {
		toast.success(m.app_account_security_passkey_deleted());
		await invalidateAll();
	}
...
async function startTotpEnable() {
	...
	if (result.error || !result.data) {
		toast.error(result.error?.message ?? m.app_account_security_totp_enable_failed());
		totpFlow = { step: 'idle' };
		return;
	}
...
async function confirmTotpEnrollment() {
	...
	if (result.error) {
		toast.error(result.error.message ?? m.app_account_security_totp_code_invalid());
	...
	toast.success(m.app_account_security_totp_enabled());
	totpFlow = { step: 'idle' };
	await invalidateAll();
}
...
async function onStepUpVerified() {
	...
	if (action === 'disable') {
		...
		if (result.error) {
			toast.error(result.error.message ?? m.app_account_security_totp_disable_failed());
		} else {
			await authClient.getSession({ query: { disableCookieCache: true } });
			toast.success(m.app_account_security_totp_disabled());
			await invalidateAll();
		}
		disabling = false;
		...
		if (result.error || !result.data) {
			toast.error(result.error?.message ?? m.app_account_security_backup_regenerate_failed());
		} else {
			freshBackupCodes = result.data.backupCodes;
			toast.success(m.app_account_security_backup_regenerated());
		}
	}
}
...
<Stack gap="6">
	{#if data.passkeysEnabled}
		...
		<Card>
			{#snippet header()}
				<h2 class="text-fluid-lg font-semibold">{m.app_account_security_heading_passkeys()}</h2>
			{/snippet}

			<p class="text-sm text-muted mb-4">{m.app_account_security_passkeys_description()}</p>
								...
								{#if renamingId === pk.id}
									<div class="flex items-center gap-2">
										<Input
											...
											placeholder={m.app_account_security_passkey_name_placeholder()}
											aria-label={m.app_account_security_passkey_rename()}
											disabled={savingRename}
										/>
										<Button size="sm" onclick={saveRename} disabled={savingRename}>
											{#if savingRename}<Spinner size="xs" class="mr-1" />{/if}
											{m.app_account_security_passkey_rename_save()}
										</Button>
								...
								{:else}
									<div class="flex items-center gap-2">
										...
										<Badge variant={pk.backedUp ? 'success' : 'outline'}>
											{pk.backedUp
												? m.app_account_security_passkey_badge_synced()
												: m.app_account_security_passkey_badge_device()}
										</Badge>
									</div>
									...
									<span class="text-xs text-muted">
										{m.app_account_security_passkey_created({ date: formatDate(pk.createdAt) })}
										{#if pk.lastUsedAt}
											· {m.app_account_security_passkey_last_used({ date: formatDate(pk.lastUsedAt) })}
										{/if}
									</span>
								{/if}
							</div>
							{#if renamingId !== pk.id}
								<div class="flex items-center gap-2">
									<Button size="sm" variant="outline" onclick={() => startRename(pk)}>
										{m.app_account_security_passkey_rename()}
									</Button>
									<Button size="sm" variant="outline" onclick={() => requestDelete(pk)}>
										<span class="i-lucide-trash-2 h-4 w-4"></span>
										<span class="sr-only">{m.app_account_security_passkey_delete()}</span>
									</Button>
								</div>
							{/if}
						</div>
			...
			{:else}
				<p class="text-sm text-muted mb-4">{m.app_account_security_passkeys_empty()}</p>
			{/if}
			...
			<div class="add-passkey">
				<Input
					...
					placeholder={m.app_account_security_passkey_name_placeholder()}
					aria-label={m.app_account_security_passkey_name_placeholder()}
				...
				<Button onclick={addPasskey} disabled={addingPasskey}>
					...
					{m.app_account_security_passkey_add()}
				</Button>
			</div>
		</Card>
	{/if}
	...
	<Card>
		{#snippet header()}
			<h2 class="text-fluid-lg font-semibold">{m.app_account_security_heading_totp()}</h2>
		{/snippet}

		{#if data.twoFactorEnabled}
			<div class="flex items-center gap-2 mb-4">
				<Badge variant="success">{m.app_account_security_totp_badge_enabled()}</Badge>
				<span class="text-sm text-muted">{m.app_account_security_totp_enabled_description()}</span>
			</div>

			{#if freshBackupCodes}
				<div class="backup-codes" role="region" aria-label={m.app_account_security_backup_heading()}>
					<p class="text-sm font-medium text-fg">{m.app_account_security_backup_heading()}</p>
					<p class="text-xs text-muted">{m.app_account_security_backup_warning()}</p>
					...
					<div class="flex gap-2">
						<Button size="sm" variant="outline" onclick={() => copyCodes(freshBackupCodes ?? [])}>
							...
							{m.app_account_security_backup_copy()}
						</Button>
						<Button size="sm" variant="outline" onclick={() => (freshBackupCodes = null)}>
							{m.app_account_security_backup_dismiss()}
						</Button>
					</div>
				</div>
			{/if}
			...
			<div class="totp-enroll">
				...
				>
					<Input
						...
						placeholder={m.app_account_security_totp_code_placeholder()}
						aria-label={m.app_account_security_totp_code_placeholder()}
					...
					<Button type="submit" disabled={!totpFlow.code.trim() || totpFlow.verifying}>
						...
						{m.app_account_security_totp_confirm()}
					</Button>
...
<ConfirmDialog
	...
	title={m.app_account_security_passkey_delete_title()}
	description={data.passkeys.length <= 1
		? m.app_account_security_passkey_delete_last_warning({ name: deleteTarget?.label ?? '' })
		: m.app_account_security_passkey_delete_description({ name: deleteTarget?.label ?? '' })}
	confirmLabel={m.app_account_security_passkey_delete()}
	destructive
