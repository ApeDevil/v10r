...
import { BUILD_LABELS, LANE_LABELS } from '$lib/showcases/ai/labels';
import { AI_LAYERS } from '$lib/showcases/ai/topology';
...
<div class="profile">
	<div class="kernel">
		<code class="kernel-entry">retrieval/retrieve()</code>
		<ul class="lanes" aria-label={m.showcase_ai_sec_retrieval()}>
		...
		<p class="tenancy-note">{m.showcase_ai_retrieval_tenancy()}</p>
	</div>
	...
	{#if surface === 'deskbot'}
		...
		<div class="sync">
			...
			<p class="sync-note">
				<code>desk.file.updatedAt</code> → <code>desk-retrieval-sync</code>
				<br />
				{m.showcase_ai_retrieval_sync()}
			</p>
		</div>
	{/if}
</div>
