...
<Stack gap="6">
		...
		<Card>
			{#snippet header()}
				<Cluster justify="between">
					<Typography variant="h5" as="h2">{m.showcase_db_storage_connection_card_status()}</Typography>
					<form
						method="POST"
						action="?/retest"
						use:enhance={() => {
					...
					>
						<Button type="submit" variant="outline" size="sm" disabled={testing}>
							...
							{m.showcase_db_storage_connection_retest()}
						</Button>
					</form>
				</Cluster>
			{/snippet}
		...
		{#if data.connected}
			...
			<Card>
				{#snippet header()}
					<Typography variant="h5" as="h2">{m.showcase_db_storage_connection_card_bucket()}</Typography>
				{/snippet}
			...
			<Card>
				{#snippet header()}
					<Typography variant="h5" as="h2">{m.showcase_db_storage_connection_card_latency()}</Typography>
				{/snippet}
	...
	{#if data.connected}
		<Cluster justify="center" class="mt-6 mb-4">
			<Button variant="outline" size="sm" onclick={() => reseedDialogOpen = true} disabled={reseeding}>
				...
				{m.showcase_db_storage_connection_reseed()}
			</Button>
		</Cluster>
	{/if}
