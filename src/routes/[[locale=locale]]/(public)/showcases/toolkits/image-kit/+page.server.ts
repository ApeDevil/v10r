...
import { getVisionProvider, loadProviderRegistry, type ProviderEntry } from '$lib/server/ai';
...
/** The vision provider for this user, or null — including when the settings cannot be read. */
async function visionProviderFor(userId: string): Promise<ProviderEntry | null> {
	try {
		return getVisionProvider(await loadProviderRegistry(), userId);
	} catch {
		return null;
	}
}
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const provider = await visionProviderFor(locals.user.id);
	return {
	...
	upload: async ({ request, locals }) => {
		...
		try {
			...
			const estimate = provider
				? estimatePreRunCost(provider.modelId, { width: result.width, height: result.height })
				: null;
