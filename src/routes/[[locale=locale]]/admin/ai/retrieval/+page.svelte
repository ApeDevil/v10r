...
import RetrievalPipelineDiagram from '$lib/components/admin/ai/RetrievalPipelineDiagram.svelte';
...
<Stack gap="6">
	...
	<Card>
		{#snippet header()}
			<h2 class="text-fluid-lg font-semibold">Retrieval Pipeline</h2>
		{/snippet}
		<RetrievalPipelineDiagram counts={pipelineCounts} />
	</Card>
	...
	<Card>
		{#snippet header()}
			<Cluster justify="between">
				<h2 class="text-fluid-lg font-semibold">Retrieval Overview</h2>
		...
		<Card>
				...
				<Cluster justify="between" align="center">
					...
					<div class="filter-bar">
						{#each statusFilters as s}
							<a
								href="/admin/ai/retrieval?status={s}"
								class="filter-link"
			...
			{#if docData.entries.length === 0}
				<EmptyState
					...
					description={data.filters.status !== 'all' ? 'No documents match this filter.' : 'No retrieval documents have been ingested.'}
				>
					{#if data.filters.status !== 'all'}
						<a href="/admin/ai/retrieval">
				...
				{#if docData.totalPages > 1}
					<div class="pagination">
						{#if data.filters.page > 1}
							<a
								href="/admin/ai/retrieval?page={data.filters.page - 1}&status={data.filters.status}"
								class="page-link"
							>
