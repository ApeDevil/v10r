<script lang="ts">
	...
	const latencyTier = $derived(
		data.latencyMs < 50 ? 'fast' :
		data.latencyMs < 200 ? 'normal' : 'degraded'
	);

	const tierVariant = $derived(
		latencyTier === 'fast' ? 'success' as const :
		latencyTier === 'normal' ? 'warning' as const : 'error' as const
	);

	const tierLabel = $derived(
		latencyTier === 'fast' ? 'Fast' :
		latencyTier === 'normal' ? 'Normal' : 'Degraded'
	);
...
<PageContainer class="py-7">
	<PageHeader
		title="Connection"
		description="Live Redis health check via Upstash HTTP REST API. The latency below was measured during your page load."
		breadcrumbs={[
			{ label: 'Home', href: '/' },
