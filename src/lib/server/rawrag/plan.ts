/**
 * plan — the pure, dependency-free chunk planner shared by the in-app RAG ingest
 ...
 * (section/paragraph targets, overlap) arrives via `PlanOptions`, never from config
 * imports — importing `./config` (which re-exports `$lib/server/config`) is exactly
 * what would re-break the Bun import, so DO NOT add it. Mirrors `markdown-split.ts`.
 */
