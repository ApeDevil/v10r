...
// `buildCallLogRow` (folded in at the bottom) reaches the Vercel platform and env.
 ...
 *
 * The schema tests prove the table rejects bad rows; `observer.test.ts` proves the builder
 ...
 * inserted through the REAL schema. It lives in this file rather than its own so it shares this
 * suite's database instead of paying for a second full schema push.
 */
describe('the private producer satisfies the constraints', () => {
	const NOW = new Date('2026-08-03T00:00:00Z');

	const producerObservation = (over: Partial<McpCallObservation> = {}): McpCallObservation => ({
		method: 'tools/call',
		toolName: 'search_patterns',
		diag: null,
		isError: false,
	...
	it('a successful private call — query, answer and workspace all kept — inserts cleanly', async () => {
		const row = buildCallLogRow(producerObservation(), producerContext, NOW);
	...
	it('a private miss and a protocol row insert cleanly too', async () => {
		const miss = buildCallLogRow(producerObservation({ isError: true, diag: 'empty' }), producerContext, NOW);
	...
	it('an oversized answer is capped by the builder BEFORE the CHECK would fire', async () => {
		const row = buildCallLogRow(
			producerObservation({ responseText: `prose ${'word '.repeat(2000)}` }),
			producerContext,
			NOW,
		);
