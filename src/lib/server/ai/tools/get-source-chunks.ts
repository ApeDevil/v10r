/**
 * get_source_chunks — drill-down into raw source chunks by ID.
 ...
 * Per-turn call cap enforced by the orchestrator (MAX_SOURCE_CHUNK_TOOL_CALLS_PER_TURN).
 */

import { jsonSchema, tool } from 'ai';
import { MAX_SOURCE_CHUNK_TOOL_IDS } from '$lib/server/llmwiki';
...
export function createGetRetrievalChunksTool(userId: string, sink?: DrilledChunkSink) {
	return {
		get_source_chunks: tool({
			description:
			...
			inputSchema: jsonSchema<ToolInput>({
				...
				properties: {
					ids: {
						...
						minItems: 1,
						maxItems: MAX_SOURCE_CHUNK_TOOL_IDS,
			...
			execute: async ({ ids }) => {
				...
				const bounded = ids.slice(0, MAX_SOURCE_CHUNK_TOOL_IDS);
