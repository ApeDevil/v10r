 ...
 * always resolves to a `DependencyResult<T>` envelope, degrading to `unavailable`
...
export async function fetchBranchStatus(): Promise<DependencyResult<NeonBranchStatus>> {
	if (!neonConfigured()) {
		return {
			status: 'unavailable',
			data: null,
