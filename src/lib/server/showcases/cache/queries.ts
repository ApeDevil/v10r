...
import { redis } from '$lib/server/cache/index';
import { SHOWCASE_CACHE_PREFIX } from './guards';
...
export async function verifyConnection(): Promise<CacheConnectionInfo> {
	...
	const keys = await r.keys(`${SHOWCASE_CACHE_PREFIX}*`);
 ...
 * are counted against YOUR bucket" is the actual lesson. `SHOWCASE_CACHE_PREFIX`
...
export async function checkRateLimit(ipKey: string): Promise<RateLimitResult> {
	...
	const ratelimit = new Ratelimit({
		...
		prefix: `${SHOWCASE_CACHE_PREFIX}ratelimit`,
		ephemeralCache,
	});
