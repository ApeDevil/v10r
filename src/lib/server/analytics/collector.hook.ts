...
export const analyticsCollector: Handle = async ({ event, resolve }) => {
	...
	if (shouldTrack) {
		...
		const consentTier = event.locals.consentTier ?? parseConsentTier(event.cookies.get(CONSENT_COOKIE));
		...
		} else if (event.cookies.get(SESSION_COOKIE)) {
	...
	//
	// Buffered in Redis, not written to Postgres: this is the one lane whose traffic
	// arrives when no human is around, and a per-hit INSERT woke the database for
	// every crawler request (see bot-hit-buffer.ts).
	if (botLane) {
