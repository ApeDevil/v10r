...
describe('desk mutations', () => {
	...
	describe('toggleFileAiContext', () => {
		it('toggles aiContext both ways — the flag decides whether a file reaches the AI', async () => {
			const f = makeFile({ userId: USER_A.id, aiContext: false });
			await db.insert(file).values(f);

			expect((await toggleFileAiContext(f.id, USER_A.id, true))?.aiContext).toBe(true);
			expect((await toggleFileAiContext(f.id, USER_A.id, false))?.aiContext).toBe(false);
		});
