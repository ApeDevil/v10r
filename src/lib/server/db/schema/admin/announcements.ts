...
export const announcementSeverityEnum = adminSchema.enum('announcement_severity', ['info', 'warning', 'critical']);
...
export const announcements = adminSchema.table(
	...
	(table) => [
		index('announcements_active_idx').on(table.startsAt, table.endsAt).where(sql`${table.active} = true`),
		index('announcements_created_desc_idx').on(table.createdAt.desc()).where(sql`${table.active} = true`),
	],
);
