...
export const themeEnum = personalizationSchema.enum('theme', ['light', 'dark', 'system']);
export const displayDensityEnum = personalizationSchema.enum('display_density', ['compact', 'comfortable', 'spacious']);
export const dateFormatEnum = personalizationSchema.enum('date_format', ['relative', 'absolute', 'iso']);

export const userPreferences = personalizationSchema.table('user_preferences', {
	userId: text('user_id')
		.primaryKey()
