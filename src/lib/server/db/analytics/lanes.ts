/**
 * Telemetry lane discrimination — which build produced a real-user sample.
 *
 ...
 * 1051 ms against an honest prod-lane 1431 ms. Contamination made the site look
 ...
 * is why `unknown` is a real lane and not an error — and why it is excluded from
 * prod-lane aggregates rather than assumed to be prod.
 */
...
export type TelemetryLane = 'prod' | 'dev' | 'unknown';
 ...
 */
export function classifyLane(targets: readonly (string | null | undefined)[]): TelemetryLane {
	let sawProd = false;
