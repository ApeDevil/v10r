...
import { classifyLane, classifyTarget, DEV_SCOPE_PATTERN, PROD_SCOPE_MARKER } from './lanes';
...
const DEV_TARGETS = [
	...
	'#workbench-dock>p.mb-fluid-4.s-2XUM8H2CQwz0.text-fluid-base.text-muted',
...
describe('classifyLane', () => {
	it('reports prod when only prod markers appear', () => {
		expect(classifyLane(PROD_TARGETS)).toBe('prod');
	});

	it('reports dev when only dev markers appear', () => {
		expect(classifyLane(DEV_TARGETS)).toBe('dev');
	});
	...
	it('reports dev when a session mixes both', () => {
		expect(classifyLane([...PROD_TARGETS, ...DEV_TARGETS])).toBe('dev');
		expect(classifyLane([...DEV_TARGETS, ...PROD_TARGETS])).toBe('dev');
	});

	it('reports unknown when nothing is classifiable, rather than assuming prod', () => {
		expect(classifyLane(UNSCOPED_TARGETS)).toBe('unknown');
		expect(classifyLane([])).toBe('unknown');
		expect(classifyLane([null, undefined])).toBe('unknown');
	});
});
