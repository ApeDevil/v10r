 ...
 * RAG :Entity/:Chunk nodes. These showcase queries run on UNAUTHENTICATED routes, so
 ...
 *
 * Sibling of `../rag/queries.test.ts` (the owner-scoped RAG side of the same leak).
 */
...
describe('showcase graph reads are demo-label-scoped (anon leak fix)', () => {
	...
	it('findShortestPath guards both endpoints AND every node on the path', async () => {
		await findShortestPath('a', 'b');
		expectAllGraphCallsDemoScoped();
		for (const [statement] of cypherMock.mock.calls) {
