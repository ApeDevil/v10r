/**
 * Tenancy lock for the retrieval graph reads (the cross-user graph-leak fix).
 *
...
const {
	getAllRetrievalEntities,
	getEntityNeighborhood,
	findShortestPath,
	expandViaGraph,
	getEntitiesForChunks,
	getRetrievalGraphStats,
...
describe('retrieval graph reads are tenant-scoped', () => {
	it('getAllRetrievalEntities scopes nodes and edges', async () => {
		await getAllRetrievalEntities(OWNERS);
	...
	it('getRetrievalGraphStats scopes the node and edge counts', async () => {
		await getRetrievalGraphStats(OWNERS);
		expectAllCallsScoped(OWNERS);
	});
});
