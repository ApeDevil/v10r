...
/**
 * Retrieval graph reads are tenant-scoped. Every function takes `ownerIds` — typically
...
export async function expandViaGraph(
	 ...
	 *  retrieval passes USER_TIMEOUT_MS so a slow Aura instance can't block a response. */
	timeoutMs?: number,
...
export async function getEntitiesForChunks(
	 ...
	 *  passes USER_TIMEOUT_MS to keep the response path from blocking on Aura. */
	timeoutMs?: number,
): Promise<EntityInfo[]> {
...
/** Get all retrieval entities and their relationships as KnowledgeData for visualization (tenant-scoped). */
export async function getAllRetrievalEntities(ownerIds: string[]): Promise<KnowledgeData> {
