...
import { USER_TIMEOUT_MS } from '$lib/server/graph/config';
import { expandViaGraph, getEntitiesForChunks } from '$lib/server/graph/retrieval/queries';
...
export async function searchGraph(
	...
	try {
		graphResults = await expandViaGraph(seedIds, [userId], Math.min(maxHops, 2), USER_TIMEOUT_MS);
	} catch {
...
export async function getGraphEntities(chunkIds: string[], ownerIds: string[]): Promise<RetrievedEntity[]> {
	...
	try {
		const entities = await getEntitiesForChunks(chunkIds, ownerIds, USER_TIMEOUT_MS);
