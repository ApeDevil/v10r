...
export async function retrieve(
	...
	const tierPromises = opts.tiers.map(async (tier) => {
		...
		try {
			...
			onEvent && emit(onEvent, stepId, 'done', {
			...
			onEvent && emit(onEvent, stepId, 'error', {
				durationMs: Math.round(performance.now() - tierStart),
	...
	onEvent && emit(onEvent, 'rank', 'done', {
	...
	onEvent && emit(onEvent, 'context', 'done', {
