/**
 * Sending a notification: create the in-app row, push it over SSE, and route it to the
 * user's external channels.
 *
 * One function, not a service object. The domain layer is plain functions everywhere else, and
...
async function sendPushNow(notificationId: string, userId: string, type: NotificationType, locale: string) {
	...
	const result = await channel.send({
		to: userId, // the push channel fans out over the user's devices itself
		subject: BRAND_NAME,
