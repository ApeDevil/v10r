...
export const wizardStep2Schema = v.object({
	street: v.pipe(v.string(), v.nonEmpty('Street is required'), v.maxLength(200)),
	city: v.pipe(v.string(), v.nonEmpty('City is required'), v.maxLength(100)),
...
export const wizardStep3Schema = v.object({
	plan: v.pipe(v.string(), v.nonEmpty('Select a plan')),
	terms: v.pipe(
		v.boolean(),
...
export const dynamicSchema = v.object({
	title: v.pipe(v.string(), v.nonEmpty('Title is required'), v.maxLength(100)),
	tags: v.pipe(
		v.array(
			v.object({
				name: v.pipe(v.string(), v.nonEmpty('Tag name is required'), v.maxLength(30)),
			})
		),
