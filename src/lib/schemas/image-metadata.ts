/**
 * Extracted image metadata + its cost estimate.
 *
 * Not under `schemas/showcase/` despite only the image-metadata showcase writing rows:
 * `server/ai/pricing.ts` and the admin cost board read `CostEstimate` from here, so a
 * `showcase/` path would misreport a production contract as demo data.
 */
