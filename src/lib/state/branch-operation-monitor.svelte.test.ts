/**
 * BranchOperationMonitor lifecycle — the leaks a browser test can't reliably catch:
...
import { BranchOperationMonitor } from './branch-operation-monitor.svelte';
...
describe('BranchOperationMonitor', () => {
	it('polls while running, then stops and fires onDone exactly once on terminal', async () => {
		...
		const m = new BranchOperationMonitor();
		m.start('r1', onDone);
