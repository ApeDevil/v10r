 ...
 *
 * `SourceChunk` is the client-facing descriptor for an original retrieval chunk the
 * assistant drilled into via `get_source_chunks`. It is streamed on assistant
 ...
 * document, the content, the source `layer`, the granularity `level`, and the
...
export interface SourceChunk {
	...
	content: string;
	/** Always 'chunks' for a drilled chunk (the honest "corpus" axis). */
	layer: 'chunks';
	level: ChunkLevel;
