<script lang="ts">
	...
	// Filter items — search across label AND hint; hide pages when no query
	let filtered = $derived.by(() => {
		if (!query) return items.filter((item) => item.type !== 'page');
	...
	// Group by type with per-group caps when query is active
	let grouped = $derived.by(() => {
		const recent = filtered.filter((i) => i.type === 'recent');
		const panels = filtered.filter((i) => i.type === 'panel');
	...
	// Flat list from capped groups for keyboard navigation
	let flatFiltered = $derived([
		...grouped.recent,
		...grouped.panels,
		...grouped.pages,
		...grouped.actions,
