...
import { type LayerId, NRAG_LAYERS, NRAG_READ_PATH, NRAG_SCAFFOLD } from './nrag-pipeline';

interface Props {
	/** Live counts per layer (null = not yet measured). */
	counts: Partial<Record<LayerId, number | null>>;
	compileScaffold?: boolean;
	lintScaffold?: boolean;
}

let { counts, compileScaffold = false, lintScaffold = false }: Props = $props();
...
<div class="pipeline">
	...
	<div class="flow" aria-hidden="true">
		{#each NRAG_READ_PATH as step, i (step.n)}
			<div class="flow-node">
				<span class="flow-n">{step.n}</span>
				<span class="flow-label">{step.label}</span>
			</div>
			{#if i < NRAG_READ_PATH.length - 1}
	...
	<div class="table-wrap" tabindex="0" aria-label="nRAG retrieval layers">
		<table class="layers">
			...
			<tbody>
				{#each NRAG_LAYERS as layer (layer.id)}
					<tr>
	...
	<div class="read-path">
		...
		<ol>
			{#each NRAG_READ_PATH as step (step.n)}
				<li><span class="step-label">{step.label}</span> — {step.detail}</li>
			{/each}
		</ol>
	</div>

	{#if compileScaffold || lintScaffold}
		<div class="scaffold">
			<span class="i-lucide-construction h-4 w-4" aria-hidden="true"></span>
			<span class="scaffold-label">Not yet live:</span>
			{#each NRAG_SCAFFOLD as s (s.label)}
