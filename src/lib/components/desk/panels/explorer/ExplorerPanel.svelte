...
import { ExplorerData } from './explorer-data.state.svelte';
import { ExplorerUploads } from './explorer-uploads.state.svelte';
import MoveToDialog from './MoveToDialog.svelte';
import type { ExplorerNode } from './node';
import type { AssetListItem, ExplorerPostItem, FileListItem } from './types';
...
function openNode(node: ExplorerNode) {
	...
	switch (node.source) {
		case 'blog-post': {
			const p = node.sourceData as unknown as ExplorerPostItem;
			openPost(p);
			break;
		}
		case 'blog-asset': {
...
function openPost(p: ExplorerPostItem) {
...
async function exportPost(p: ExplorerPostItem) {
	try {
