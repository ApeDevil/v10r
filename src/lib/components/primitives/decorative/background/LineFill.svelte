...
let spacing = $derived(variant === 'crosshatch' ? Math.max(spacingProp, 16) : Math.max(spacingProp, 12));

// Enforce opacity cap: 0.5 for scanline/hatch, 0.35 for crosshatch
let opacity = $derived(variant === 'crosshatch' ? Math.min(opacityProp, 0.35) : Math.min(opacityProp, 0.5));

// Default angle: 0 for scanline, 45 for hatch/crosshatch
let angle = $derived(angleProp !== 0 ? angleProp : variant === 'scanline' ? 0 : 45);
