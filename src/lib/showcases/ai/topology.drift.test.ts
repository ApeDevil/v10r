...
import { RETRIEVAL_STEPS } from '$lib/types/retrieval-trace';
...
describe('registration maps', () => {
	it('STEP_LAYER covers exactly the pipeline registry', () => {
		// Compile-time exhaustive already (Record<RetrievalStepId, …>); pin the runtime view too.
		for (const step of RETRIEVAL_STEPS) expect(STEP_LAYER[step.id]).toBeDefined();
	});
