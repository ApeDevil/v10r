/**
 * The deskbot page's second recorded turn — the tool-layer gate firing WITHOUT a plan — an
 * AUTHORED stand-in in the persisted shape (`InspectedTurn`). The model reached for a direct
 * edit; `desk_edit_markdown` returned the `requiresApproval` sentinel instead of writing, the
 * turn stopped on a one-step proposal, and nobody has decided yet (`pending`). Different
 * claim from `deskbot-plan`: here the TOOL refused, before any human was asked — even a
 * single-passage edit cannot run in-loop.
 *
 * Real and authored parts as in `deskbot-plan.ts`; the awareness blocks are the composer's
 * output for this one-panel workspace. Nothing was mutated. Replace with
 * `scripts/ai/record-turn-fixture.ts --surface deskbot --out <this file>`; ids are `demo_`
 * by construction.
 */

import type { InspectedTurn } from '../inspector';

export const deskbotSentinel: InspectedTurn = {
	provenance: {
		kind: 'authored',
		recordedAt: '2026-09-12',
	},
	question: 'Fix the typo in notes.md — change "recieve" to "receive".',
	trace: {
		messageId: 'demo_turn',
		...
		blocks: [
			...
			},
			{
				...
				capability: 'desk-awareness',
				section: 'awareness',
				text: '<desk-context>\n<panel type="markdown" label="notes.md" file_id="demo_file_1" file_type="markdown" version="7">\n# Meeting notes\n\nWe agreed to recieve the vendor\'s revised quote by Friday and to review it together on Monday.\n\n</panel>\n</desk-context>',
				chars: 248,
				stable: false,
			},
			{
		...
		toolExecutions: [
			{
				...
				output: {
					...
					target: {
						...
						name: 'notes.md',
						version: 7,
						updatedAt: '2026-09-12T08:12:51.000Z',
					},
				},
		...
		proposal: {
			...
			status: 'pending',
			riskTier: 'medium',
			goal: 'Edit 1 passage in "notes.md"',
			steps: [
				{
					action: 'Edit 1 passage in "notes.md"',
					tool: 'desk_edit_markdown',
					risk: 'write',
					rationale: 'Edit 1 passage in "notes.md"',
					recovery: 'revision',
					retentionDays: 90,
					target: {
						fileId: 'demo_file_1',
			...
			receipts: [],
			failureMessage: null,
			expiresAt: '2026-09-12T08:29:10.000Z',
			approvedAt: null,
			executedAt: null,
		},
	...
	answer: 'notes.md is open, so the fix is one passage. The edit needs your approval before anything is written.',
};
