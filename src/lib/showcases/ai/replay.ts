 ...
 * the old retrieval trace), never incremental mutation, so step-back/Home/End are safe and
...
/**
 * A recorded pipeline step event — `RetrievalStepEvent` minus the fields a fixture
 * must never carry (`requestId` correlates real requests; nothing else is sensitive).
 */
export type RecordedStepEvent = Omit<RetrievalStepEvent, 'requestId'>;
...
function seedSteps(surface: AiSurface): RetrievalTraceStep[] {
	if (surface !== 'chatbot') return [];
	return RETRIEVAL_STEPS.filter((d) => !d.dynamic && (d.engine === 'llmwiki' || d.engine === 'both')).map((d) => ({
		id: d.id,
		instanceKey: d.id,
		label: d.label,
		phase: d.phase,
...
function applyStepEvent(steps: RetrievalTraceStep[], event: RecordedStepEvent): void {
	...
	const registry = RETRIEVAL_STEPS.find((d) => d.id === event.step);
	steps.push({
		id: event.step,
		instanceKey: key,
		label: registry?.label ?? event.step,
		phase: event.phase ?? PHASE_OF[event.step],
		retriever: event.retriever ?? registry?.retriever,
		engine: registry?.engine ?? 'llmwiki',
		status: event.status,
...
function deriveLayers(input: {
	...
	for (const step of input.steps) {
		...
		const layer = STEP_LAYER[step.id as RetrievalStepId];
